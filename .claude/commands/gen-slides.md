...
根据作品目录下的原始文稿，生成一套符合规范的交互式教学动画，保存为 `index.html`。
...
**主内容源：`source.md`（原始文稿，动画内容的全量来源）**

读取 `<works_dir>/<作品目录名>/source.md`。

- 若存在 → 作为动画内容的**主要输入**，包含完整的概念、数据、例子、论证链条
- 若不存在 → 降级读取 `script-full.md`；若也不存在，再读取 `script.md`，并在输出时提示用户：
  > ⚠️ 未找到 `source.md`，已降级使用口播稿作为内容源。建议执行 `/gen-script` 时保存原始文稿，以提升动画信息密度。

**结构参考：`script.md`（口播稿，仅用于 Slide 分段映射和提词器同步）**

读取 `<works_dir>/<作品目录名>/script.md`。

- `script.md` 的作用：**确定 Slide 数量、分段边界、`{step}` 触发位置**
- `script.md` 的内容**不是**动画的内容来源——它是为「说」压缩过的口语稿，信息量不足
- 若 `script.md` 不存在，提示：
  > 未找到口播稿，请先执行 `/gen-script <作品目录名>` 生成口播稿（用于确定 Slide 结构）。

**两者分工说明：**

| 文件 | 在 `/gen-slides` 中的角色 | 原因 |
|------|--------------------------|------|
| `source.md` | **内容源** — 从中提炼每张 Slide 展示什么、用什么例子、有哪些数据和论证 | 原始文稿信息全量，适合高密度动画 |
| `script.md` | **结构锚点** — 决定几张 Slide、每张的主题边界、`{step}` 对应的动画节奏 | 口播稿已按认知节奏切分，结构清晰 |
...
按 `script.md` 的 Slide 分段结构，结合 `source.md` 的完整内容，逐张诊断：

**4.0 内容映射（在诊断前必须先做）**

将 `source.md` 的内容按 `script.md` 的 Slide 分段边界切分，为每张 Slide 整理出：
- **核心论点**：这张 Slide 要让观众理解/记住什么（从 source.md 提炼）
- **可用素材**：source.md 中与该 Slide 主题相关的数据、例子、类比、论证链条
- **补充细节**：source.md 中口播稿没提但可以视觉化展示的额外信息（这正是信息密度的来源）
...
- [ ] **动画内容来自 `source.md`，而非 `script.md`**（确认每张 Slide 的数据/例子/细节均取自原始文稿）
- [ ] **`source.md` 中有价值的补充信息已视觉化**（不因口播稿没提就丢弃）
- [ ] 无 `setInterval` 无限循环
