...
横版使用自由布局（左右分栏、网格等）。
竖版**必须从 VERTICAL.md 的 7 种布局变体中选择**：
...
4. **步骤纵向型** — 流程/步骤
5. **数据焦点型** — 数据展示
6. **双列卡片网格** — 多要点高密度（≥4 块）★ 优先使用
7. **三列摘要+底部总结** — 章节总结/核心提炼 ★ 优先使用

**每张 Slide 在诊断阶段选定布局变体**，记录在诊断表中。

### 差异 5：默认主题 — 深色信息流（主题 9）

竖版默认使用 **主题 9「深色信息流」**（STYLES.md），除非用户明确指定其他主题。

**主题 9 专属要求：**
- 品牌层（`.brand-layer`）**每张 slide 必须包含**：右上角品牌名、左上角 slide 标签、右下角页码、左下角来源
- 标题关键词必须用 `.kw-*` 高亮（蓝/橙/紫/绿/红），每个标题 1~2 个关键词
- 每页信息密度 ≥ 4 个信息块（卡片/列表项/数据点）
- 优先使用 **变体 6（双列卡片网格）** 和 **变体 7（三列摘要+底部总结）**，内容页首选变体 6
- 卡片必须有 3px 彩色左边框，相邻卡片颜色不重复
- 封面页使用变体 1（居中冲击型），但背景仍为深色
...
- [ ] 动画方向已从「左→右」转为「下→上」
- [ ] 默认使用主题 9「深色信息流」（除非用户指定其他）
- [ ] 品牌层（`.brand-layer`）每张 slide 均包含
- [ ] 标题关键词已用 `.kw-*` 高亮
- [ ] 每页信息密度 ≥ 4 个信息块
- [ ] 内容页优先使用变体 6/7
